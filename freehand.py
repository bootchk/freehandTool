...
This is free software, covered by the GNU General Public License. See the file COPYING for details.
'''
...
Compare with other freehand tools, which draw (not renders) pixels until end of tool,
then fits splines to the complete pointer track and renders the spline.
Here, immediately draw vector graphics (splines and lines.)
Here, look at only a finite tail of the pointer track.
...
(leaving gaps in the input pointer track).  In other words,  the input will have low resolution.
...
- vectors (straight lines) fitting end to end and quaranteed to pass through every pointer position (the pixel around it.)
...
potrace
=======
Uses sequence of filters and algorithms from potrace library for tracing bitmap images, by Peter Selinger.
...
That is, for a COMPLETE pointer path, there are many fits, and it finds the best.
Incrementally, we don't have a complete pointer path,
and we don't have the computing power to incrementally generate a best fit for the partial pointer path.
We only find the easiest fit, the first one generated.
This could be extended to find a better fit from a set of alternative fits for a short tail of the pointer path
...
Here detectCorner works despite gaps, i.e. isolates rest of pipe from gaps in the pointer path. 
...
That is a problem for incremental pointer path tracing: when user moves the pointer very fast,
it leaves long gaps in the pointer track, makes for long path lines, and cusps rather than splines.
...
The timing of a user's stroke has meaning.
When a user strokes rapidly, they mean a smooth curve.
When a user slows the stroke and turns, they mean a cusp.
But a slow diagonal generates many PathTurns, which should not generate a cusp.

Ghosting
=======
Since the pipeline lags, ghost a straight line from last output of pipeline to current pointer position.
...
Terminology
===========
Use separate terms for distinct concepts!
Pixels have corners (between sides of a square.)
A pointer position is usually coordinates of the upper left corner of a pixel.
Pointer paths, often called strokes, are sequence of captured pointer positions.
...
(Pointer track is used ambiguously to mean the real pointer track and the pointer path?)
Pointer paths have turns (between subpaths on axis.) Called corners in potrace.
Path lines are between one or more turns.
Path lines have pivots (points between sequential, non-aligned vectors.)
(Line is used ambiguously to mean a path line or a graphic path item that is an element of type line?)
Graphic path items (of type line or spline) have end points.
A cusp is an end point between two graphic element lines.  Also called a corner in potrace.
We distinguish between the pointer path (input) and the graphic path (output.)

Pointer means "pointing device", i.e. mouse, pen, or touch, (or cursor.)
...
# Elapsed time TODO
# Determines whether slow pointer movement generates cusp
SLOW = 100
...
def nullLine(point):
  ''' Return a zero length line at a point. '''
...
class FreehandTool(object):
  ...
  def __init__(self, scene, view):
    ...
    # Ghost ungenerated tail of pointer track with line
  ...
  def closeFilterPipe(self):
    '''
    Close generators. 
    They will finally generate SOME of final objects (i.e. turn, line) to current pointer position.
  ...
  def pointerPressEvent(self, event):
    ...
    # Create contiguous pointer track in a new single QGraphicPathItem
  ...
  def pointerReleaseEvent(self, event):
    ...
    '''
    CurveGenerator only finally draws to midpoint of current line.
    Add final segment to path, a line from midpoint to current pointer position.
    Note path already ends at the midpoint, don't need to "return" it from close()
    (and close() CANNOT return a value.)
    
    If last curve generated was MidToEnd, we might not need this,
    but that might leave end of pointer track one pixel off.
    '''
    self.path.addItem( (self._scenePositionFromEvent(event), ))
    # TODO draw a spline, or OW fit it better
  ...
  def TurnGenerator(self, startPosition):
    '''
    Takes pointer position on explicit call to send().
    ...
    try:
      while True:
        ...
        if turn is not None:
          ...
          countPositionsSinceTurn = 0
          """
          elif countPositionsSinceTurn > 10:
            '''
            Send a turn after foo positions regardless of axis.
            This addresses the problem whereby sharp turns on the axis
            to not generate a curve even after many position
            '''
            self.lineGenerator.send(position) # !!! a turn is just a position
            previousPosition = position  # Roll forward
            countPositionsSinceTurn = 0
          """
        else: # path is still on an axis: wait
          # countPositionsSinceTurn+=1
          pass
    finally:
  ...
  def LineGenerator(self, startPosition):
    ...
    Consumes turns until pixels of pointer path cannot be approximated by (impinged upon by) one vector.
    ...
    try:
      while True:
        ...
        if line is not None:  # if turn not satisfied by vector
        ...
        elif positionElapsedTime > SLOW:
          # User turned slowly, send a forced line which subsequently makes cusp-like turn
          ...
          startTurn = previousTurn  # !!! current turn is part of next line
        # else current path (all turns) still satisfied by a line: wait
      ...
      if previousTurn != startTurn:
        ''' Have turn not sent. Fabricate a line and send() it now. '''
  ...
  def CurveGenerator(self, startLine):
    ...
    On startup, previous line is nullLine (!!! not None), but this still works.
    '''
    previousLine = startLine  # null line
    state = None
    ...
    try:
      while True:
        ...
        if isLineForced:
          ...
          state = LAST_GENERATED_TO_END
          previousLine = nullLine(pathEndPoint) # !!! next curve from midpoint of nullLine
        else:
          ...
          state = LAST_GENERATED_TO_MID
          previousLine = line  # Roll forward
        
        # Add results to graphicItem for pointer track.
        for item in curves:
          # self.scene.addItem(item)  # add new path comprising this segment
          self.path.addItem(item) # add segment to existing path
        
        self.pathTailGhost.updateStart(pathEndPoint)  # Update ghost to start at end of path
      ...
      ''' 
      Last drawn curve stopped at midpoint of line.
      Caller must draw one last element from there to current pointer position.
      Here we don't know pointer position, and caller doesn't *know* line midpoint,
      but path stops at last line midpoint.  IOW  midpoint is *known* by caller as end of path.
  ...
  def lineFromPath(self, startTurn, previousTurn, currentTurn, constraints, directions=None):
    ...
    calculate exactly which pointer position (between turns) violated constraints.
    '''
    '''
    I found that for pointer tracks, this happens so rarely it is useless.
  ...
  def forceLineFromPath(self, startTurn, previousTurn, currentTurn, constraints, directions=None):
    ...
    Note this is a PathLine, not a GraphicLine.
    '''
    constraints.__init__()
  ...
  Fit a curve to two vectors.
  '''
  
  def nullcurveFromLines(self, line1, line2):
    ''' 
    Return QGraphicsItem to represent tail of pointer track.
    FOR TESTING. Generates a simple line (instead of a smoothed curve.)
    After all, a straight line is a curve with null curvature.
    '''
    return (QGraphicsLineItem(line1), ) # Note lag one line
  ...
  def curvesFromLineMidToMid(self, line1, line2):
    ...
    # aliases for three points defined by two abutting lines
    point1 = line1.p1()
    point2 = line1.p2()
    point3 = line2.p2()
    ...
    if alpha > ALPHAMAX:
      ...
      '''
      return self.createSpline(controlPoint1=self.interval(0.5+0.5*alpha, point1, point2), 
  ...
  def createLinePathElement(self, endPoint):
    ''' 
    Create our representation of a LinePathElement from current end of path to endPoint.
    Mangle to signature of AddingGraphicsPathItem.addItem()
    A LinePathElement is a tuple comprising single endpoint of line.
    (the starting point of LinePathElement is implicitly the end of the graphic path.)
    '''
    return (endPoint, )
  ...
  def createCusp(self, cuspPoint, endPoint):
    '''
    Create sharp cusp. Return two straight line path segments,
...
class PointerTrackGhost(object):
  ...
  Line between current pointer position and last pointer track path segment generated, which lags.
...
class AddingGraphicsPathItem(QGraphicsPathItem):
  ...
  def addItem(self, item):
    ''' 
    Add segment (line or spline) to path.
    Item is tuple:
    - line: (endpoint)
